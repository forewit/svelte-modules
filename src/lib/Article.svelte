<script lang="ts">
  ...
  function markdownToHtml(markdown: string): string {
    ...
    // extract and combine each css block into a single string
    const css = `<style>${markdown
      .match(/```css[\s\S]*?```/g)
      ?.map((block) => block.replace(/```css/, "").replace(/```/, ""))
      .join("")}</style>`;
    ...
    return css + markdown
      ...
      .join("")
      .replace(/!\[(.+?)\]\((.+?)\)/g, `<img src="${base}$2" alt="$1" />`)
